...
export function createEnemies(path, count = 4) {
  ...
  // Space them out, each patrolling a short stretch it never leaves.
  const creatures = []
  for (let i = 0; i < count; i++) {
    const centre = Math.floor(((i + 0.5) / count) * path.length)
    const half = 12 + Math.floor(hash2(i * 3.7, i * 1.9) * 10)
    creatures.push({
      from: Math.max(0, centre - half),
      to: Math.min(path.length - 1, centre + half),
      at: centre,
